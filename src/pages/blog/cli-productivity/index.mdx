...
looking at one right now: `bash` – the default
...
is rich with benifits. `zsh` is more customizable, the prompts are more 
versatile, and the completion speed is faster than `bash`. 

In addition, `bash` cannot handle floating-point arithmetic i.e. 
`number=$((3.1/4.7))` or arrays[^1]. Support for both while scripting gives `zsh`
the edge. It also has unicode support! 😊
...
– functions. This simplifies shell interaction
...
feature which is absent in `bash` or `zsh` by default[^2].
...
[^1]: While `bash` has "arrays", it doesn't implement true array data structures.
Under the hood [they are (index, value) linked lists](https://github.com/lucasscharenbroch/bash-with-floats/blob/master/array.c#L5).
`zsh` implements true arrays capable of pointer arithmetic. With larger datasets, the difference in efficiency is notable.
...
A similar file exists for `fish`, located at ~/.config/fish/config.fish. Here is a sample:
...
active terminal connection and K spawned terminal sessions. With `tmux` you
...
## neovim
Neovim is a fork of Vim which expands on it's functionality by allowing you to
"drop in" community-curated plugins to enhance your experience.
...
in a public[^3] GitHub repository. 
...
[^3]: We opt for a public repository so you can clone it on any machine without
